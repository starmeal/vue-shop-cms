        ...
        <el-form :model="form" :rules="rules" ref="form" :size="size" label-width="200px" class="demo-form">
          ...
          <el-form-item v-if="status == null || status == 2" label="授权书等其他材料" prop="powerOfAttorneyImg">
            <div class="flex">
              <div class="kuake">
                <div class="div-img"
                     v-for="(item,index) in form.powerOfAttorneyImg"
                     :key="index">
                  <div class="bg-marsk">
                ...
                <el-upload
                        v-if="form.powerOfAttorneyImg.length<5"
...
<script>
  ...
  import { getRodio,shopNominateInfo,shopNominateSubmit } from "@/api/shopNominate/index";
  export default {
    name: "index",
    data(){
      return {
        ...
        rules: {
          ...
          powerOfAttorneyImg: [
            {required: true, message: '请上传授权书等其他材料', trigger: 'change'}
          ]
        }
      }
    },
    ...
    methods: {
      ...
      async getRadio(){
        let {body:{list0}} = await getRodio({type:'store_type'});
        this.radioArr = list0;
        ...
        this.$refs[formName].validate(async(valid) => {
          if (valid) {
            ...
            this.$nextTick(() => {
              ...
              document.querySelectorAll(".el-form-item__error")[0].scrollIntoView(obj);
            });
            console.log('错误提交');
            return false;
          }
